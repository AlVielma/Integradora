...
$validacionreceta= new ValidacionesReceta();
$errores=[];
extract($_POST);
if(isset($_POST['aceptarr']))
{

  if($validacionreceta->nulo([$nombre,$edad,$diagnostico,$plasticos,$rx_armazon,$armazon,$totalPedido,$material]))
  {
    $errores[]="Los campos nombre,edad,diagnostico, plastico, rx Armazon, material y total de pedido  deben llenarse";
  }
  if(!$validacionreceta->mayor6($edad))
  {
  ...
  else
  {
    ...
    $url = 'receta.php?errores=' . urlencode($erroresJson);
    header('Location: ' . $url);
    exit();
  }

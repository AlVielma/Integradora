...
class metodoscita
{
    ...
    public function mostrar()
    {
        ...
        cc.ultimo_examen,cc.uso_gotas FROM Citas_Cliente cc inner join Usuarios u on u.id = cc.usuario WHERE activo=1");
    ...
    ,$uso_gotas)
    {
        ...
        sintomas_oculares, enfermedades_oculares, lentes_actualmente, armazon, contacto, ultimo_examen, uso_gotas,activo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,1)");
    ...
    {
        $query = $this->conectar->prepare("UPDATE Citas_Cliente SET activo=0 WHERE id=?");
        $query->execute([$id]);
    }
